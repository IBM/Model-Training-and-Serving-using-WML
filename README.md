...
``` 
To continously monitor the logs of Training Run
...
Once a training run has completed successfully, the trained model can be permanently stored into the repository from where it can be later deployed for scoring. To do this use the command bx ml store training-runs <model-id>:
